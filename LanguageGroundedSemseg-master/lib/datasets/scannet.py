...
class ScannetVoxelizationDataset(VoxelizationDataset):
    ...
    def test_pointcloud(self, pred_dir, num_labels):
        ...
        for i, data_path in enumerate(self.data_paths):
            ...
            print(room_id, i)
            # pred = np.load(glob.glob(pred_dir + f"/{i}.npy")[0])
            # pred = np.load(os.path.join(pred_dir, 'scene%04d_%02d.npy' % (i, 0)))

            # # Scale with voxel size
            # pred[:, :3] *= self.voxelizer.voxel_size

            # # pred = np.load(os.path.join(pred_dir, 'scene_%04d_%02d.npy' % (i, 0)))
            ...
            np.savetxt(f'{eval_path}/{room_id}.txt', result, fmt='%i')
