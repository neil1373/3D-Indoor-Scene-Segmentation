...
def checkpoint(model, optimizer, epoch, iteration, config, best_val=None, best_val_iter=None, postfix=None):
    ...
    json.dump(vars(config), open(
        config.log_dir + '/config.json', 'w'), indent=4)
...
def feat_augmentation(data, normalized, config):
    ...
    if data.size(1) > 3 and random.random() < 0.9:
        data[:, -1] += torch.randn((data.size(0)),
                                   dtype=data.dtype).type_as(data)

    # normal jitter
    if data.size(1) > 6 and random.random() < 0.9:
        data[:, 3:6] += torch.randn((data.size(0), 3),
                                    dtype=data.dtype).type_as(data)
...
def save_predictions(coords, upsampled_pred, transformation, dataset, config, iteration,
    ...
    # Normally batch_size, may be smaller at the end.
    iter_size = coords[:, -1].max() + 1
    ...
    for i in range(iter_size):
        ...
        if dataset.IGNORE_LABELS:
            ...
            #     masked2label = [label2masked.tolist().index(i)
            #                     for i in range(maskedmax)]
            #     orig_pred = np.take(masked2label, orig_pred)
            decode_label_map = {}
            for k, v in dataset.label_map.items():
                decode_label_map[v] = k
            orig_pred = np.array([decode_label_map[x]
                                  for x in orig_pred], dtype=np.int)
        # Determine full path of the destination.
        full_pred = np.hstack(
            (orig_coords[:, :3], np.expand_dims(orig_pred, 1)))
