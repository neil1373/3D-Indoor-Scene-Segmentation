...
def save_instance(segment_points, label_id, cat_name, scene_id, base_path, segment_faces=None, limited_annotation_points=None):
    ...
    labbelled_instance_points = np.append(
        segment_points[:, :6], labels, axis=1)
    ...
    save_point_cloud(labbelled_instance_points, out_file,
                     with_label=True, verbose=False, faces=segment_faces)
...
def load_pcd(path):
    ...
    feats = np.array([data['red'], data['green'],
