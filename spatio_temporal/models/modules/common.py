                ...
                if axis_type == ME.RegionType.HYPER_CUBE:
                    for offset in region_offset:
                        for curr_offset in range(curr_kernel_size):
                            off_center = (
                                int(math.floor((curr_kernel_size - 1) / 2)
                                    ) if center else 0
                            )
...
def conv(in_planes,
    ...
    region_type, axis_types, kernel_size, region_offset = convert_conv_type(
        conv_type, kernel_size, D, tensor_stride=t_stride, up_stride=up_stride, dilation=t_dilation)
...
def conv_tr(in_planes,
    ...
    t_dilation = torch.IntTensor([dilation, ] * D)
    region_type, axis_types, kernel_size, region_offset = convert_conv_type(
        conv_type, kernel_size, D, tensor_stride=t_stride, up_stride=t_up_stride, dilation=t_dilation)
    kernel_generator = ME.KernelGenerator(
        kernel_size,
        upsample_stride,
        dilation,
